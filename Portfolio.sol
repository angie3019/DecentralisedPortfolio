...
//0xF2d8D6b4B8f807456b96b2F96B040882e70Ba939
contract Portfolio{
  ...
  Project[6] public projects;
  Education[3] public educationDetails;
  Experience[6] public experienceDetails;

  string public imageLink="QmPnAqCduDvFSDueyonsJ5hQEpGJKVz9SeqxtM2hfPr9zQ";
  string public description="Blockchain Freelancer and Trainer with 10 years experience in the software industry actively contributing in Web 3.0 space by building state-of-the-art Dapps and Smart contracts.";
  string public resumeLink="QmSssqP6wuaFMXynVsFm7CKv8ojY299bsS3C2MTqr1MNe9";
  uint projectCount;
  uint educationCount;
  ...
  function insertProject(string calldata _name,string calldata _description,string calldata _image,string calldata _githubLink) external{
      require(projectCount<6,"Only 6 projects allowed");
  ...
  function changeProject(string calldata _name,string calldata _description,string calldata _image,string calldata _githubLink,uint _projectCount) external{
       require(_projectCount>=0 && _projectCount<6,"Only 6 projects allowed");
      projects[projectCount] = Project(_projectCount,_name,_description,_image,_githubLink);
  }
    function allProjects() external view returns(Project[6] memory){
        return projects;
    }
...
//Experience
    ...
    function insertExperience(string calldata _date,string calldata _post,string calldata _knowledgeAcquired,string calldata _companyName) external  onlyManager{
     require(experienceCount<6,"Only 6 education details allowed");
    ...
    function changeExperience(string calldata _date,string calldata _post,string calldata _knowledgeAcquired,string calldata _companyName,uint _experienceDetailCount) external  onlyManager{
     require(_experienceDetailCount>=0 && _experienceDetailCount<6,"Invalid experienceCount");
     experienceDetails[_experienceDetailCount]=Experience(_experienceDetailCount,_date,_post,_knowledgeAcquired,_companyName);
    }
    
    function allExperienceDetails() external view returns(Experience[6] memory){
